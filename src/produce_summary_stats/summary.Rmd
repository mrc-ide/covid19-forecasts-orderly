...
```{r}
by_phase <- read_csv("unwtd_pred_summary_by_phase.csv")
```
### 50% CrI
...
- The relative error of the model was smaller than the error a linear 
...
```{r}
no_change_compare <- readRDS("no_change_compare.rds")
```

Summary of weekly relative error in weeks when the performance was no
better than the no-change model.

```{r}
x <- no_change_compare[no_change_compare$err_level == "greater_than_1", ]
z <- stack(summary(x$weekly_rel_err))
z$values <- signif(z$values, digits = 3)
```

...
adapt_tuning <- function(repli, I, N_geo, within_iter, theta0, sigma, SI, mu0,
  ...
  for (i in 1:repli) {
    ...
    res <- MCMC_iter(
      incidence = I, N_geo = N_geo, iter = within_iter,
      theta0 = theta0, s = new_sigma, SI = SI, mu0 = mu0,
      over_disp = over_disp, upper_log_i0, lower_log_i0, lower_r,
      upper_r
    )
    ...
    new_sigma <- adapt(
      theta = res$theta, s = new_sigma, it = within_iter
    )

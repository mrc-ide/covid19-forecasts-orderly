...
-	likely stabilising^1^  in `r nrow(stable)` countries <br/>
(`r stable$country`);
-	growing slowly^2^ in `r nrow(growing_slow)` countries <br/>
(`r growing_slow$country`);
-	growing rapidly^3^ in `r nrow(growing_fast)` countries <br/>
...
1. if the 95% Credible Interval (CrI) of $R_t$ contains 1.
2. if the 2.5^th^ percentile of $R_t$ is greater than 1 and the
97.5^th^ percentile is less than 2. 
3. if the 97.5^th^ percentile of $R_t$ is greater than 2.

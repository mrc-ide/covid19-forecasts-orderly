...
week_starting <- as.Date('14-06-2020', format = '%d-%m-%Y')

```

# Introduction 

As of 21^st^ June 2020,  more than
8,700,000 cases of COVID-19 have been reported across the world, with more than 461,000
deaths [@whositrep]. 
...
ascertainment) for 53 countries with active transmission.
...
two weeks. For the week beginning 21^st^ June 2020, 
the number of countries/regions included based on these thresholds is 53.
...
transmission for the week ending 21^st^ June 2020. A country is defined
...
phase as stable/growing slowly. If the the width of the 95% CrI is
...
a * are based on an unweighted ensemble of Models 1, 2 and 3.
Results from individual models are shown
in the section Methods. The reporting of deaths and cases in Brazil is currently changing; results should be interpreted with caution.
...
*Figure 13.*: Temporal trends in the ratio of reported
...
delay) reaches 1. The reporting of deaths and cases in Brazil is currently changing; results should be interpreted with caution.
...
*Figure 14.* Projections (7-day ahead) for the week starting 21^st^ June 2020 from individual models
...
```
*Figure 15.* Projections (7-day ahead) for the week starting 21^st^ June 2020 from individual models (Models 1, 2
...
*Figure 16.* Projections (7-day ahead) for the week starting 21^st^ June 2020 from individual models (Models 1, 2
...
```
*Figure 17.* Projections (7-day ahead) for the week starting 21^st^ June 2020 from individual models (Models 1, 2
...
```
*Figure 18.* Projections (7-day ahead) for the week starting
21^st^ June 2020 from individual models (Models 1, 2
and 3) for each country in South America with
active transmission (see Methods). For each model,
the solid line shows the median and the shaded region shows the 95%
CrI of the projections. The reporting of deaths and cases in Brazil is currently changing; results should be interpreted with caution.
...
```
*Figure 19.* Estimates of $R_t$ from individual models for each country in Europe with
...
*Figure 20.* Estimates of $R_t$ from individual models for each country in Asia with
active transmission (see Methods) for the week starting
21^st^ June 2020. 
...
```
*Figure 22.* Estimates of $R_t$ from individual models for each country in
North America with
active transmission (see Methods) for the week starting
21^st^ June 2020. 
...
```
*Figure 23.* Estimates of $R_t$ from individual models for each
...
*Notes* 
Some countries have been exlcuded from the analysis despite meeting
...
reported by the World Health Organization (https://covid19.who.int)

- The number of deaths in India was manually corrected to 594 on
  17^th^ June as deaths that occurred in March and April were reported 
  on this day.
  
- The number of deaths in Russia was manually corrected to 182, 181
  and 161 on 19^th^, 20^th^ and 21^st^ June respectively.
  
- The number of deaths in Iraq was manually corrected to 69 on 20^th^
  June, and 88 on 21^st^ June.

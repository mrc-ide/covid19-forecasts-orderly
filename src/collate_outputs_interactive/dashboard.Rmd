...
long_forecasts <- readRDS("long_forecasts.rds")
long_forecasts_rt <- readRDS("long_forecasts_rt.rds")
long_forecasts_ps <- readRDS("long_forecasts_ps.rds")


rt_qntls <- readRDS("unweighted_rt_qntls.rds")
rt_qntls <- rt_qntls[rt_qntls$si == "si_2", ]
...
forecast_weeks <- seq(
  as.Date("2020-03-08"), to = as.Date("2020-11-16"), by = "7 days"
)
...
country_restimate <- reactive({
  ...
  restim <- tidyr::spread(restim, key = quantile, value = out2)
...
country_long_ps <- reactive({
  long_forecasts_ps[long_forecasts_ps$country == input$country &
                    long_forecasts_ps$forecast_week %in% input$forecast_week, ]
...
output$long <- renderPlotly({
  ...
  obs_country <- country_obs()
  pred <- dplyr::left_join(pred, obs_country, by = c("date" = "dates"))
...
renderPlotly({
  key <- country_pred()$country # This will uniquely identify tracts for Plotly
  ps_country <- country_long_ps()
  p1b <- ggplot(ps_country) +
    geom_ribbon(
      aes(
        x = date, ymin = `2.5%`, ymax = `97.5%`, group = forecast_week
      ),
    ...
    geom_ribbon(
      aes(
        x = date, ymin = `25%`, ymax = `75%`, group = forecast_week
      ), alpha = 0.5
    ) +
    geom_line(
